...
| Introduction | We have evaluated other base models such as Qwen2.5-32B-Instruct, which is widely adopted in related work. It also shows good performance compare to base models. The results are summarized in Table 1. |
| ------------ | ------------------------------------------------------------ |
| **Analyze**  | It also achieves strong performance compared to other base models, demonstrating that our method generalizes effectively across different models. However, the Qwen-based version does not surpass the performance of our primary ReasonFlow model, fine-tuned based on QwQ. Thus, we selected the QwQ version for presentation in the 32B model size in our paper.|
...
| Introduction | For the SFT stage, we aim to enable the LLM to perform structured RAG and achieve an in-depth understanding of the templates in the library. Based on that, we conduct additional ablation experiment on SFT stage. We randomly sampled 100 thought templates along with their corresponding example problems. Using o3-mini, we generated 10 variant problems for each example, ensuring they differed from the originals while still assessing the same underlying knowledge and skills, as metioned in Appendix B. Then we evaluate the average retrieving accuracy among the base models and our SFT models. Here we define two metrics to comprehensively evaluate the structued RAG ability gained after the SFT stage. The structure accuracy is the percentage that the generated conetent has the correct structued format that could be correctly parsed and continue to the retrieval phase, the retrieving accuracy is the percentage that the models correctly retrieve the corresponding template for the problem after successfully parsed the structured query. **We present the results in Table.4**|
| ------------ | ------------------------------------------------------------ |
| **Analyze**  | We can find that through our first stage SFT, both structure accuracy and retrieving accuracy has significantly improved. This indicates that the first-stage SFT effectively enhances the structured output capability of the LLMs, enabling more accurate structured RAG. The improved retrieval accuracy further signifies that models develop a deeper understanding of the relationship between problems and corresponding templates. |
...
| Introduction | For the RL stage, we aim to utilize the reward signal from downstream reasoning tasks to optimize the ability of LLMs to generate correct and logical reasoning trajecotris. We conducted an intuitive ablation experiment to demonstrate the effectiveness and improvements brought by the template trajectory reward stage. We sampled 100 example problems and utilzie the model after the SFT stage and the model that experienced both SFT and RL stage to retrieve thought templates and genrate corresponding reasoning trajectories as high-level guidelines for inference LLM to instiate. Here we add another metric which is the reasoning accuracy of the infernce LLM that reflects the quality of the reasoning trajectories, **we present the results in Table.5** |
| ------------ | ------------------------------------------------------------ |
| **Analyze**  | We can see that after the RL stage, all three metrics has improved, especially the reasoning accuracy of the downstream inference LLMs. This clearly shows that the RL stage effectively enhances the quality of generated reasoning trajectories, significantly boosting the final reasoning accuracy. Additionally, we observed improvements in both structure accuracy and retrieval accuracy. We attribute this to the fact that the RL stage not only reinforces the model's ability to generate logically coherent reasoning trajectories but also implicitly encourages adherence to the desired structured format. In other words, by optimizing the reward signal based on downstream reasoning performance, the model is driven to produce outputs that are **both logically consistent and structurally well-formed**. This dual reinforcement helps reduce format errors and improves template retrieval, ultimately resulting in enhanced overall performance. |
...
| 4). $\pi_{sft} $ is used without reference| We sincerely thank you for pointing it out. In the equation.4 we use $\pi_{sft}$ to substitute $\pi_{struct}$, which is a more noraml expression of the reference model in the loss function of DPO. Since we previously mentioned that $\pi_{\theta}$ is initialized from $\pi_{struct}$, we initially assumed it would be intuitive for readers that $\pi_{sft}$ represents $\pi_{struct}$. However, we will explicitly clarify this in the final version according to your advice. |
